...
class WorkoutDetailViewController: UIViewController {
    ...
    let setCell = "SetCell"
    weak var delegate:TabbedDelegate?
    ...
    func addToViewButtonTapped(fromButton button:AddToViewButtonTapped) {
        ...
        case .calendar:
            if let tabBarController = tabBarController as? MainTabBarController {
                ...
                delegate?.passData(forWorkout: workoutSelected)
                tabBarController.selectedIndex = button.getSelectedIndex()!
            }
        case .journal:
           // let splitViewController = tabBarController?.viewControllers?[1] as! UISplitViewController
           // let datesVC = splitViewController.viewControllers[0] as! UINavigationController
             tabBarController?.selectedIndex = button.getSelectedIndex()!
